# Fixed Kenneth French Data Parser
# Complete rewrite to fix scoping issues

module KFParserFixed

using HTTP, CSV, DataFrames, Dates, Statistics, Printf

export download_kf_factors, parse_kf_csv_content, parse_kf_date_string

"""
Parse Kenneth French date format YYYYMM to Date.
Examples: 196307 -> 1963-07-01, 202412 -> 2024-12-01
"""
function parse_kf_date_string(date_str::String)::Date
...
"""
function parse_kf_csv_content(csv_content::String)::DataFrame
    ...
    for (i, line) in enumerate(data_lines)
        try
            ...
            if length(parts) >= 7  # Date + 6 factors
                # Parse date - use the function directly by name
                date_str = strip(parts[1])
                date = parse_kf_date_string(date_str)  # Direct call without module prefix
...
function download_kf_factors(
    ...
    try
        ...
        cd(temp_dir) do
            ...
            factors = parse_kf_csv_content(csv_content)
